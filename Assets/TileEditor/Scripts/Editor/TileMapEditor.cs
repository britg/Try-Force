...
public class TileMapEditor : Editor
{
	...
	void OnSceneGUI()
	{
		...
		if (editing)
		{
			...
			cursorZ = Mathf.RoundToInt(mousePosition.y / tileMap.tileSize);
	...
	void DrawGrid()
	{
		...
		for (int x = -gridSize; x <= gridSize; x++)
		{
			for (int z = -gridSize; z <= gridSize; z++)
			{
				...
				var p = new Vector3((cursorX + x) * tileMap.tileSize, (cursorZ + z) * tileMap.tileSize, 0);
	...
	void DrawRect(int x, int z, int sizeX, int sizeZ, Color outline, Color fill)
	{
		...
		var min = new Vector3(x * tileMap.tileSize - tileMap.tileSize / 2, z * tileMap.tileSize - tileMap.tileSize / 2, 0);
		var max = min + new Vector3(sizeX * tileMap.tileSize, sizeZ * tileMap.tileSize, 0);
		rect[0].Set(min.x, min.z, 0);
		rect[1].Set(max.x, min.z, 0);
		rect[2].Set(max.x, max.z, 0);
		rect[3].Set(min.x, max.z, 0);
